...
export default function TechStackCarousel() {
  ...
  return (
    <div className="overflow-hidden py-12 bg-black">
      ...
      {/* Row 1 - Scrolls Left */}
      <div className="relative mb-12 marquee-container py-6">
        <div className="flex animate-marquee-left gap-16">
          {[...row1Companies, ...row1Companies].map((company, idx) => (
            <div key={idx} className="flex-shrink-0 logo-wrapper min-w-[140px] flex items-center justify-center">
              <img 
                src={company.logo} 
                alt={company.name}
                className="h-14 w-auto object-contain tech-logo"
                loading="lazy"
                onError={(e) => {
      ...
      {/* Row 2 - Scrolls Right */}
      <div className="relative marquee-container py-6">
      ...
      <style jsx>{`
        ...
        .logo-wrapper:hover .tech-logo {
          filter: brightness(1) invert(0) opacity(1) drop-shadow(0 0 20px #00BFFF);
          transform: scale(1.2);
        }
      `}</style>
    </div>
